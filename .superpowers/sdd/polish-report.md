...
| 前端 | `npm --prefix frontend run test:unit`；`frontend/node_modules/.bin/tsc --noEmit -p frontend/tsconfig.json`；`npm --prefix frontend run build` | 42 files、195 tests 通过；TypeScript、public 和 monitor 构建通过。 |
...
- 依赖方向保持 `interfaces/infrastructure → application → domain`；没有业务 panic 或新增业务 goroutine。双 Gin engine 启用 request logger/recovery，supervisor goroutine 通过 `safeServe` recover。
- privacy sentinel/fail-open 覆盖 IP、Visitor header、query/body、数据库路径、密钥和原始错误不外泄；public listener 不暴露 monitor/API。
- monitor bundle 是 613.92kB，保留既有 Vite >500kB 提示；没有引入任何新大型依赖。
