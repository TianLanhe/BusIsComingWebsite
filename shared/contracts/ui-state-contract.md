...
| 状态 | 触发 | 展示 | 行为 |
|------|------|------|------|
| `carousel-auto` | 首屏进入且无交互 | 单一主手机截图，左右低强调预览 | 约 3 秒切换 4 个功能页，10 秒内至少切换 2 次 |
| `carousel-paused` | hover、focus、drag、touch | 当前主图保持稳定 | 暂停自动切换，交互结束后恢复或保持可控状态 |
| `carousel-dragging` | 手机滑动或桌面拖动 | 相邻截图或相邻功能页横向进入 | 不显示底部缩略图、胶片条、图片按钮组或常驻箭头 |
...
- 视觉模式必须为 `cinematic-phone-rail`：一个主手机截图 + 左右低强调预览。
