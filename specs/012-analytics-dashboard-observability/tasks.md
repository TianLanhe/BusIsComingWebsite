...
审查修复（2026-07-24）：已补充六个 SQLite probe 各自失败时的字段保留测试、`processStartedAt` 缺失时保留 Dropped 的测试、loopback 配置接受/拒绝矩阵及 API 实际地址注入测试；同时补充全部字段可用的 E2E、字节/时长自适应格式和删除壳层静态监听器状态的回归。
...
审查修复（2026-07-24）：T079 已补 `limit=1` 第一页/第二页 cursor 回归，明确摘要及会话元数据使用完整历史、仅 timeline events 严格分页；T080 断言四卡 DOM 顺序和 `eventCount=2` 配合单页事件 fixture；T081–T082 改为真实点击侧栏、抽屉和底栏并覆盖语言切换的已应用/草稿/筛选/compare 未选中/P50/Visitor 状态；T086–T087 补 390 顶部与安全区视觉回归、三语操作文案和全量验证证据。
