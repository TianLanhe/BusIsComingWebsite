...
技术路线采用前后端分离的 Web 应用结构。第一版只实现前端静态主页和共享内容契约，不启用实时查询后端；在线查询区明确为静态演示。共享契约会先定义首页内容、下载平台状态和 UI 状态，为后续真实在线查询服务、下载 manifest 或内容服务接入保留边界。

## 技术背景

**前端语言/版本**：TypeScript，面向现代浏览器的 ES module 构建；具体 TypeScript 版本由实现阶段的包管理文件锁定。

**后端语言/版本**：第一版不运行后端服务；后续服务端边界按 Node.js LTS 运行时规划，真实在线查询和下载资源服务作为独立 feature 接入。

**主要依赖**：React + Vite 用于新建前端应用；CSS Modules 或等价 scoped CSS 管理样式；lucide-react 或等价图标库用于按钮、导航、功能项和状态图标；不引入大型 UI 套件，避免偏离已确认 Figma 视觉。
...
**目标平台**：手机浏览器、桌面浏览器、静态托管前端；后续可接入独立 API 服务。
...
**前后端契约**：功能级契约位于 `specs/001-homepage-v1/contracts/`；后续实现应把稳定版本复制或映射到 `shared/contracts/`。契约包括首页内容 schema、下载 manifest schema 和 UI 状态契约。第一版无实时 API 错误格式，下载资源不可用时使用 manifest 中的不可用状态和用户可读原因。
...
| 前后端分离与契约优先：边界、契约和错误格式已记录 | 通过 | 第一版无实时后端，但前端、未来后端和共享契约边界已记录；契约位于 `contracts/`。 |
...
**结构决策**：第一版只实现静态前端和共享契约，因此 `backend/` 只需要记录未来服务边界，不启动服务端代码；`shared/contracts/` 在实现阶段承接本 feature 的合同文件，供前端和未来后端共同引用。该决策不违反前后端分离原则，因为前端不会硬编码未来后端内部规则，在线查询也不会伪装为实时服务。

## 复杂度跟踪

无宪法违规。需要注意的复杂点是“第一版不运行后端服务”，但原因是在线查询已明确为静态演示，真实查询后端属于后续 feature；本计划通过共享契约保留服务边界。
...
| 前后端分离与契约优先 | 通过 | `contracts/` 已定义内容、下载和 UI 状态契约；后端 README 作为后续边界任务。 |
