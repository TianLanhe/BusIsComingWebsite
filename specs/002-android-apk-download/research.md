...
## 决策 2：服务端采用 DDD bounded context 分层

**Decision**：后端新增 `downloads` bounded context，并按 `domain`、`application`、`infrastructure`、`interfaces` 组织代码。当前 APK、APK 元数据、校验错误和下载结果属于领域层；下载当前 APK 的用例和端口属于应用层；受管文件读取、元数据读取和 SHA-256 计算属于基础设施层；Gin handler、路由和错误映射属于接口适配层。

**Rationale**：用户明确要求服务端代码采用 DDD 目录组织结构，项目宪法也新增 DDD 原则。即使当前功能只有一个下载端点，APK 校验、文件替换、错误映射和后续发布管理都会继续演进，提前固定依赖方向可以避免把领域规则写进 HTTP handler。

**Alternatives considered**：

- 单包 `internal/downloads`：文件更少，但容易把路由、文件系统和校验规则混在一起。
- 按技术层 `handler/service/repository`：比单包清晰，但不能充分表达领域概念和用例边界。
- 按版本目录组织：适合发布归档，但用户已确认只保留当前 APK。
...
## 决策 4：稳定下载入口为 `/api/downloads/android/latest`
