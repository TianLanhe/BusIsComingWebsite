...
## Decision: 轮播采用 cinematic phone rail，而不是修补现有截图堆叠

**Rationale**：用户明确指出上次方案与实现偏离，关键问题是“大图底部几张小图”的缩略图堆叠。cinematic phone rail 把视觉焦点限制为一个主手机截图，并只允许左右低强调预览暗示可滑动，能把“优雅、丝滑、非缩略图控件”的意图转成可验收约束。
...
## Decision: 自动播放按 4 个功能页切换，同功能页截图由用户手动滑动

**Rationale**：4 个功能页对应现有产品叙事，自动播放保持信息节奏稳定。同功能页多张截图如果自动逐张切，会让用户以为这是杂乱图片轮播，并增加文案与截图不同步风险。
