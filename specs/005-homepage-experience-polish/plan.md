...
| 现代界面与可视化评审：UI 讨论和展示有图片、截图、设计稿或可视化 mock | 通过 | 已有 Superpowers mock；本计划新增 Figma 插件设计源和后续 visual-review 截图要求。 |
| 电脑与手机双端一致可用：布局、交互和内容展示同时覆盖手机与电脑 | 通过 | 桌面 1440px 与手机 390px 轮播和联系入口均进入 quickstart 验证。 |
| Figma 驱动的前端规格：前端/UI 功能已有 Figma 文件或链接作为后续阶段参考 | 通过 | 已记录目标 Figma 文件和本地插件；因 MCP 不可用不伪造节点 ID，tasks 前需导入插件并回填节点。 |
...
| 可验证交付与自动提交：验证命令、浏览器检查和本次 Spec Kit skill 后提交策略已定义 | 通过 | quickstart 定义 Vitest、build、Playwright、Figma 插件和截图验收；本 plan 完成后提交。 |
...
无宪法违规。需要接受的复杂度来自两点：一是 Figma MCP 写入工具当前不可用，只能生成本地插件作为可复现设计源；二是轮播必须支持自动播放、手动滑动/拖动、暂停和减少动态效果，不能用简单图片替换。被拒绝的更简单方案包括只把间隔改为 3 秒、只隐藏底部缩略图、只替换邮箱、或继续使用 launcher 背景图标；这些方案无法防止再次偏离用户确认的 A 方案，也无法满足项目 Figma、三语和视觉验收门禁。

## 第 0 阶段输出

- [research.md](./research.md)：轮播交互、手势与可访问性、logo 资产管线、香港繁中文案策略、Figma 插件 fallback、契约与验证策略决策。
...
- [figma.md](./figma.md)：目标 Figma 文件、插件导入流程、计划节点名称和回填要求。
- [figma-plugin/](./figma-plugin/)：用于在 Figma 中生成本功能桌面/手机/状态设计节点的本地插件。
...
| Figma 驱动的前端规格 | 通过 | 提供目标 Figma 文件和本地插件；MCP 不可用限制已记录，tasks 前需导入并回填节点。 |
