...
**约束**：[例如 三语 i18n、`zh-Hant` 香港实用书面语、`en` 自然克制产品表达、现代简洁优雅、移动端适配、无密钥下发、外部 API 降级 或 NEEDS CLARIFICATION]

**规模/范围**：[例如 页面数量、API 数量、语言数量、内容来源范围、明确排除项 或 NEEDS CLARIFICATION]

**i18n 范围**：必须覆盖 `zh-Hant`、`zh-Hans` 和 `en`，并说明 `zh-Hant` 香港本地语气、
`en` 英语产品语气和非机械直译审校方式

**前后端契约**：[记录 API 或共享契约位置、版本策略和错误格式]

**OpenAPI 接口文档**：[涉及服务端 HTTP API 时记录 OpenAPI 3.1 YAML 路径、共享沉淀路径、中文 API UI 输出路径、lint/bundle/预览方式；不涉及服务端 API 时写 N/A]

**服务端 DDD 边界**：[涉及服务端代码时记录 bounded context、领域层、应用层、基础设施层、接口适配层和依赖方向；不涉及服务端代码时写 N/A]

**服务端稳健性与可观测性**：[涉及服务端代码时记录 panic 禁止策略、HTTP recovery、goroutine recover 包装、日志字段、敏感信息脱敏和验证方式；不涉及服务端代码时写 N/A]

**代码注释与可读性**：[涉及代码实现时记录需要中文注释说明的复杂领域规则、算法、错误映射、外部约束、状态转换或降级策略；仅文档/UI 设计时写 N/A]
...
| OpenAPI 驱动的服务端接口文档：服务端 HTTP API 已有 OpenAPI 3.1 YAML、中文 API UI、共享沉淀路径和验证方式 | [通过/未通过] | [OpenAPI 路径、中文 API UI 输出、lint/预览命令或 N/A 原因] |
| 三语国际化：所有用户可见文字覆盖 `zh-Hant`、`zh-Hans`、`en`，且 `zh-Hant` 与 `en` 已按自然语气审校、未机械直译 | [通过/未通过] | [i18n 文件、内容系统、语气审校记录或验证方式] |
...
| 服务端稳健性与可观测性：panic/recover、协程安全和脱敏日志策略已定义 | [通过/未通过] | [HTTP recovery、goroutine recover、日志字段或 N/A 原因] |
| 中文注释与代码可读性：复杂逻辑、领域规则和边界处理已有中文注释策略 | [通过/未通过] | [需注释的模块、文件或 N/A 原因] |
