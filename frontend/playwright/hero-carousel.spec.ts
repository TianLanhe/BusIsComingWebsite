...
test("carousel keeps the expected order and preserves state after locale switch", async ({ page }, testInfo) => {
  ...
  await clickExposedBackCard(page, "favorite-citybus-routes-2");
  ...
  await expect(page.getByTestId("screenshot-rail")).toHaveAttribute("data-active-image-id", "route-comparison-1");
