...
export function EventTable({ items, locale, onViewVisitor }: { items: EventDetail[]; locale: MonitoringLocale; onViewVisitor: (visitorID: string) => void }) {
  return <div className="event-table-wrap"><table className="event-table"><thead><tr><th>{detailText(locale, "eventTime")}</th><th>{detailText(locale, "eventType")}</th><th>{detailText(locale, "visitorId")}</th><th>{detailText(locale, "outcome")}</th><th>{detailText(locale, "latency")}</th><th>{detailText(locale, "details")}</th></tr></thead><tbody>{items.map((event) => <tr key={event.eventId}><td>{formatDate(event.occurredAt, locale)}</td><td><code>{eventLabels[event.eventType][locale]}</code></td><td><span className="visitor-short">{truncateVisitorID(event.visitorId)}</span></td><td><span className={`outcome-badge ${event.outcome}`}>{event.outcome}</span></td><td>{event.durationMs}ms</td><td><button type="button" onClick={() => onViewVisitor(event.visitorId)}>{detailText(locale, "viewVisitor")}<ArrowRight size={13} /></button></td></tr>)}</tbody></table></div>;
}
