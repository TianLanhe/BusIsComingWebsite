    ...
    <main className="monitor-main">
      <header className="monitor-topbar">
        ...
        <div className="topbar-controls">
          <label className="monitor-control"><Clock3 size={15} /><select value={filters.rangeDays} onChange={(event) => filters.setRangeDays(Number(event.target.value))} aria-label={t("dateRange")}><option value={7}>{t("range7")}</option><option value={30}>{t("range30")}</option><option value={90}>{t("range90")}</option></select></label>
