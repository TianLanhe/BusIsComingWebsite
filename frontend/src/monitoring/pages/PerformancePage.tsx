...
export function PerformancePage({ loadPerformance = fetchPerformance }: { loadPerformance?: (query: AnalyticsQuery, signal?: AbortSignal) => Promise<PerformanceData> }) {
  ...
  return <DashboardShell active="performance" title="performance" subtitleText={detailText(locale, "performanceSubtitle")} generatedAt={data?.meta.generatedAt}>{loading || error || !data || data.meta.state !== "ready" ? <DetailState loading={loading} error={error} noData={!data || data.meta.state !== "ready"} locale={locale} /> : <>
    ...
    <section className="detail-grid-2"><article className="dashboard-card compact-card"><DetailHeading title={detailText(locale, "latencyTrend")} note="nearest-rank" /><div className="latency-bars">{data.endpoints.map((endpoint) => <div key={endpoint.operationId}><span>{eventLabels[endpoint.eventType][locale]}</span><i style={{ width: `${Math.min(100, (endpoint.p95Ms ?? 0) / Math.max(1, ...data.endpoints.map((item) => item.p95Ms ?? 0)) * 100)}%` }} /><b>{endpoint.p95Ms ?? "—"}ms</b></div>)}</div></article><article className="dashboard-card compact-card"><DetailHeading title={detailText(locale, "failures")} note="controlled categories" /><DistributionChart items={data.failures} locale={locale} /></article></section>
  </>}</DashboardShell>;
}
