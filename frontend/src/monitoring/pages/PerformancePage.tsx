...
function PercentileCell({ value, comparison, compare, locale }: { value: number | null; comparison: PercentileComparison; compare: boolean; locale: "zh-Hans" | "zh-Hant" | "en" }) {
  ...
  if (state.viewState === "no_current") return <span>—</span>;
  if (state.viewState === "disabled") return <span>{formatMS(value, locale)}</span>;
  if (state.viewState === "no_previous") return <span>{formatMS(value, locale)}<small>{monitoringCopy(locale, "comparisonMissing")}</small></span>;
  if (state.viewState === "unchanged") return <span>{formatMS(value, locale)}<small>{monitoringCopy(locale, "comparisonUnchanged")}</small></span>;
  const arrow = state.direction === "up" ? "↑" : "↓";
  const delta = comparison.deltaMs == null ? "—" : `${comparison.deltaMs > 0 ? "+" : ""}${comparison.deltaMs} ms`;
  return <span className={`latency-comparison ${state.outcome}`}>{formatMS(value, locale)}<small>{arrow} {delta}</small></span>;
}
