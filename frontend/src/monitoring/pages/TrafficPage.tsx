...
export function TrafficPage({ loadTraffic = fetchTraffic }: { loadTraffic?: (query: AnalyticsQuery, signal?: AbortSignal) => Promise<TrafficData> }) {
  ...
  return <DashboardShell active="traffic" title="traffic" subtitleText={detailText(locale, "trafficSubtitle")} generatedAt={data?.meta.generatedAt}>
    {loading || error || !data || data.meta.state !== "ready" ? state : <>
      <section className="metric-grid detail-metrics"><MetricCard label={monitoringCopy(locale, "pv")} metric={metrics.get("pv")} locale={locale} compareEnabled={data.meta.compare} /><MetricCard label={monitoringCopy(locale, "uv")} metric={metrics.get("uv")} locale={locale} compareEnabled={data.meta.compare} /><MetricCard label={monitoringCopy(locale, "successfulPlaceUV")} metric={metrics.get("successfulPlaceVisitors")} locale={locale} compareEnabled={data.meta.compare} /><MetricCard label={monitoringCopy(locale, "successfulRouteUV")} metric={metrics.get("successfulRouteVisitors")} locale={locale} compareEnabled={data.meta.compare} /></section>
