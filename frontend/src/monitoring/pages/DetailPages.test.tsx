...
describe("detail workspaces", () => {
  ...
  it("loads a visitor from the header-only identifier and announces copy feedback", async () => {
    ...
    expect(loader).toHaveBeenCalledWith("abcdefghijklmnopqrstuv", 50, undefined);
  });
});
...
function traffic(): TrafficData { return { meta, metrics: [], series: [], trialFunnel: { key: "trial", sessionGapMinutes: 30, stages: [] }, heatmap: Array.from({ length: 168 }, (_, i) => ({ weekday: Math.floor(i / 24) + 1, hour: i % 24, eventCount: i % 7, uv: i % 4 })), locales: [], devices: [], sources: [] }; }
...
function performance(): PerformanceData { return { meta, metrics: [], endpoints: [{ operationId: "queryRouteOptions", eventType: "route_query", requestCount: 10, successRate: .9, p50Ms: 200, p95Ms: 600 }], latencySeries: [], failures: [] }; }
function system(): SystemData { return { generatedAt: meta.generatedAt, database: { state: "available", rowCount: 100, sizeBytes: 4096, lastSuccessfulWriteAt: meta.generatedAt }, process: { startedAt: meta.from, droppedSinceStart: 0 }, privateListener: { state: "available", bindAddress: "127.0.0.1:18081", publicProxy: false } }; }
function visitor(): VisitorData { return { generatedAt: meta.generatedAt, timezone: "Asia/Hong_Kong", visitor: { visitorId: "abcdefghijklmnopqrstuv", firstSeenAt: meta.from, lastSeenAt: meta.to, eventCount: 2, sessionCount: 1, commonLocale: "zh-Hant", commonDeviceType: "mobile", commonSourceType: "direct" }, sessions: [{ ordinal: 1, startedAt: meta.from, endedAt: meta.to, durationMs: 100, eventCount: 2, events: [] }], pageInfo: { limit: 50, nextCursor: null, hasMore: false, totalCount: 2 } }; }
