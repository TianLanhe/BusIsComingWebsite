...
describe("PerformancePage", () => {
  it("renders six metrics, P50/P95 chart, failure distribution and endpoint table", async () => {
    ...
    expect(screen.getByRole("list", { name: "图例" })).toHaveTextContent("P50");
    expect(screen.getByRole("list", { name: "图例" })).toHaveTextContent("P95");
...
function performance(): PerformanceData { return { meta, metrics: [{ key: "requestCount", value: 12, previousValue: null, delta: null, deltaRate: null }, { key: "requestSuccessRate", value: .75, previousValue: null, delta: null, deltaRate: null }, { key: "p50Ms", value: 120, previousValue: null, delta: null, deltaRate: null }, { key: "p95Ms", value: 480, previousValue: null, delta: null, deltaRate: null }], endpoints: [{ operationId: "queryRouteOptions", eventType: "route_query", requestCount: 12, successRate: .75, p50Ms: 120, p95Ms: 480 }], latencySeries: [{ bucketStart: meta.from, bucketEnd: meta.to, eventType: "route_query", requestCount: 12, p50Ms: 120, p95Ms: 480 }], failures: [{ key: "external_timeout", count: 3, ratio: 1 }] }; }
function system(): SystemData { return { generatedAt: meta.generatedAt, database: { state: "available", rowCount: 100, sizeBytes: 4096, lastSuccessfulWriteAt: meta.generatedAt }, process: { startedAt: meta.from, droppedSinceStart: 2 }, privateListener: { state: "available", bindAddress: "127.0.0.1:18081", publicProxy: false } }; }
