...
describe("PerformancePage", () => {
  ...
  it("labels endpoint comparison states and SLI numerator/denominator in Chinese", async () => {
    renderPage(<PerformancePage loadPerformance={async () => performance()} loadSystem={async () => system()} />);
    expect(await screen.findByText("成功 PV / 总 PV")).toBeInTheDocument();
    expect(screen.getByText("上期为零，仅显示绝对变化")).toBeInTheDocument();
    expect(screen.getAllByText("暂无当前数据")).toHaveLength(2);
  });
});
...
const meta = { from: "2026-07-20T00:00:00+08:00", to: "2026-07-21T00:00:00+08:00", timezone: "Asia/Hong_Kong" as const, granularity: "day" as const, compare: true, comparisonFrom: "2026-07-19T00:00:00+08:00", comparisonTo: "2026-07-20T00:00:00+08:00", appliedFilters: { locale: [], device: [], source: [], outcome: [], platform: [], versionName: [], versionCode: [], eventType: [] }, generatedAt: "2026-07-21T01:00:00Z", state: "ready" as const };
function performance(): PerformanceData { return { meta, metrics: [{ key: "requestCount", value: 12, previousValue: 10, delta: 2, deltaRate: .2 }, { key: "requestSuccessRate", value: .75, previousValue: .7, delta: .05, deltaRate: .07 }, { key: "p50Ms", value: 120, previousValue: 110, delta: 10, deltaRate: .09 }, { key: "p95Ms", value: 480, previousValue: 480, delta: 0, deltaRate: 0 }], endpoints: [{ operationId: "queryRouteOptions", eventType: "route_query", requestCount: 12, successRate: .75, p50Ms: 120, p50Comparison: { currentMs: 120, previousMs: 0, deltaMs: 120, deltaRate: null }, p95Ms: 480, p95Comparison: { currentMs: 480, previousMs: 480, deltaMs: 0, deltaRate: 0 } }, { operationId: "downloadLatestAndroidApk", eventType: "download_request", requestCount: 0, successRate: null, p50Ms: null, p50Comparison: { currentMs: null, previousMs: 90, deltaMs: null, deltaRate: null }, p95Ms: null, p95Comparison: { currentMs: null, previousMs: null, deltaMs: null, deltaRate: null } }], latencySeries: [{ bucketStart: meta.from, bucketEnd: meta.to, eventType: "route_query", requestCount: 12, p50Ms: 120, p95Ms: 480 }], sliSeries: ["page_view", "place_query", "route_query", "download_request"].map((eventType, index) => ({ bucketStart: meta.from, bucketEnd: meta.to, eventType: eventType as "page_view" | "place_query" | "route_query" | "download_request", successfulPV: index === 1 ? 0 : 1, totalPV: index === 3 ? 0 : 1, successRate: index === 1 ? 0 : index === 3 ? null : 1 })), failures: [{ key: "external_timeout", count: 3, ratio: 1 }] }; }
