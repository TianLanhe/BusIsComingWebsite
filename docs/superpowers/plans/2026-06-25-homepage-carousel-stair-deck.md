...
Expected: FAIL because `ScreenshotStack` still exposes the previous rail mode, back cards are not buttons, and scene dots are spans.
...
await page.locator('button[data-image-id="home-all-routes-sheet"]').click();
